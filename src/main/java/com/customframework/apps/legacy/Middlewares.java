...
public class Middlewares {
  public static final Middleware adminCookieMiddleware =
      (req, next) -> {
        boolean isAdmin =
  ...
  public static final Middleware globalSanitizerMiddleware =
      (req, next) -> {
  ...
  public static final Middleware requestIdMiddleware =
      (req, next) -> {
  ...
  public static final Middleware logMiddleware =
      (req, next) -> {
        long start = System.currentTimeMillis();
        String userAgent = req.getHeaders().getOrDefault("User-Agent", "Unknown");
        Logger.log(
            String.format(
                "Routing Request: %s %s | Agent: %s", req.getMethod(), req.getPath(), userAgent));
        ...
        Logger.log(
            String.format(
                "Response Status: %d | Time: %d ms | Type: %s",
                resp.getStatusCode(), duration, resp.getContentType()));
        return resp;
      };

  public static final Middleware securityHeadersMiddleware =
      (req, next) -> {
        Response resp = next.handle(req);
  ...
  public static final Middleware referrerPolicyMiddleware =
      (req, next) -> {
        Response resp = next.handle(req);
